...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	def nIteration(self, n):
		nEnd = self.loop + n
		while (not self.lastIteration and self.selectedAction != self.actionTerminate and self.loop < nEnd):
			self.loop += 1
			self.oneIteration()
		p.postProcess()
...
# N iteratations
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	def TargetsUpdate(self, targets):
		np.reshape(targets, (targets.size,1))
		self.TargetsPseudoLinear = self.targets
		self.basisTargets = np.dot(self.basis.T, self.targets)
		self.fullStatistics()
...
#=================================================================================================================
#=================================================================================================================
#                       ESTO SERIA EL METODO ITERATIVO PARA AJUSTAR LO SINTETIZADO + SISTEMATICOS
#=================================================================================================================
#=================================================================================================================
#=================================================================================================================

#AHORA LO QUE HAY QUE HACER ES COMBINAR EL lib_me CON EL METODO rvm PARA AJUSTAR
#CON EL PRIMERO LA SENAL
#CON EL SEGUNDO LOS POSIBLES SISTEMATICOS

#EL PRIMER PASO POR TANTO CONSISTE EN DETERMINAR UN PERFIL DE STOKES DEBIDO A 
#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
...
pl.subplot(2,2,1)
pl.plot(lambv,stokIo, 'b')
pl.plot(lambv,stokIp, 'r')
pl.subplot(2,2,2)
pl.plot(lambv,stokQo, 'b')
pl.plot(lambv,stokQp, 'r')
pl.subplot(2,2,3)
pl.plot(lambv,stokUo, 'b')
pl.plot(lambv,stokUp, 'r')
pl.subplot(2,2,4)
pl.plot(lambv,stokVo, 'b')
pl.plot(lambv,stokVp, 'r')
pl.show()


#DEFINIMOS UNA BASE DE POSIBLES FUNCIONES:
#Basis = np.sin(2.*np.pi*lambv)
#Basis = np.vstack([Basis, np.cos(2.*np.pi*lambv)])
#Basis = np.vstack([Basis, np.sin(2.*np.pi*lambv/(lambv[-1]-lambv[1]))])
#Basis = np.vstack([Basis, np.cos(2.*np.pi*lambv/(lambv[-1]-lambv[1]))])
Basis = np.ones(npoint)
indexp = 0
for index in range(24):
	indexal = index % 3
	Basis = np.vstack([Basis, (np.linspace(0,npoint,npoint)-indexp)**(indexal+1)])
	if ((index+1) % 3): indexp += npoint / 4.

#PROBLEMA, CADA UNO LLEVA SUS SISTEMATICOS, PERO QUEREMOS LA MISMA Mneg PARA TODOS:

(ndum, mdum) = np.shape(Basis)
BasisS = np.zeros((ndum*4,mdum*4))

for index1 in range(ndum*4l):
	index2 = index1 % 4
	BasisS[index1,mdum*index2:mdum*(index2+1)] = Basis[index1/4,:]

Basis = BasisS.T

#TENEMOS UNA MATRIZ DE 7x31

			#Basis = np.exp(-(XStenflo-C)**2 / basisWidth**2)
			#Basis = np.hstack([Basis, np.ones((1,N)).T])

#HAY UN PROBLEMA CON LOS N Y M
N = npoint


#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
#AHORA QUE TENGO UNA PRIMERA ESTIMACION DE MI SENAL,
#SE LO QUITO A LA OBSERVACION E INTENTO AJUSTAR EL RUIDO
#Y TENGO TAMBIEN LA BASE DE VECTORES QUE REPRESENTAN MIS SISTEMATICOS
rvminI = stokI - stokIp
rvminQ = stokQ - stokQp
rvminU = stokU - stokUp
rvminV = stokV - stokVp
...
while (conditrvmme != 1):
	...
	# Do some plots
	#pl.plot(np.concatenate([lambv, lambv, lambv, lambv]), np.concatenate([stokI, stokQ, stokU, stokV]), 'ro')
	#pl.plot(np.concatenate([lambv, lambv, lambv, lambv]), np.reshape(np.concatenate([stokI, stokQ, stokU, stokV]) - np.dot(Basis, p.wInferred).T, 4l*N))
	#pl.show()


	#AHORA VIENE EL PROBLEMA Y ES QUE TENGO LA Mneg Y SUPONGO QUE AHORA EL y-me ES EL y OBSERVADO MENOS EL AJUSTE LOGRADO CON EL rvm
	#Y ADEMAS HAY QUE DERIVAR ESE PRODUCTO DE TRES MATRICES RESPECTO DE CADA UNA DE LOS PARAMETROS ATMOSFERICOS:
	# (y-me)T
	# Mneg
	# (y-me)
	#DONDE Mneg NO DEPENDE DE LOS PARAMETROS ATMOSFERICOS

	system = np.resize(np.dot(Basis, p.wInferred).T, 4l*N)
	...
	print "B={0:7f} - AlD={1:10.7f} - Wm={2:10.3f} - O={3:7.3f} - F={4:7.3f} - Br={5:8f} - a={6:6.2f} - E={7:6.2f}".format(\
	iBmag, ideltalambdaD, iomegam, (itheta / 180. * np.pi) % 360., (ichi / 180. * np.pi) % 360., ibr, iadamp, ieta0)
	

	Bmag = iBmag
	deltalambdaD = ideltalambdaD
	omegam = iomegam
	theta = itheta
	chi = ichi
	br = ibr
